...
impl Request {
    ...
    pub fn read<R: Read>(r: &mut R) -> io::Result<Self> {
        ...
        let req = PrivMessage::decode_length_delimited(&buf[0..bytes_read])?;
        let err = Error::new(ErrorKind::InvalidData, "Invalid request.");
        match req.sum {
            Some(Sum::PubKeyRequest(pkr)) => {
                let pkr = PubKeyRequest::try_from(pkr).map_err(|_e| {
                    // dbg!(_e);
                    err
                })?;
            ...
            Some(Sum::SignVoteRequest(svr)) => {
                let svr = SignVoteRequest::try_from(svr).map_err(|_e| {
                    // dbg!(_e);
                    err
                })?;
            ...
            Some(Sum::SignProposalRequest(spr)) => {
                let spr = SignProposalRequest::try_from(spr).map_err(|_e| {
                    // dbg!(_e);
                    err
                })?;
